...
export const generateCpptRalan = async () => {
  const doc = new jsPDF();
  const img = new Image();
  img.src = "/logo-rs.png";
  ...
  return new Promise((resolve) => {
    img.onload = async () => {
      ...
      // === HEADER TABEL ===
      const colWidths = [20, 12, 18, 120, 32];
...
const padding = 3;
const startX = xPos + padding;
let currentY = yPos + 4; // padding atas
const maxWidth = colWidths[3] - padding * 2;
const lineHeight = 4;

const baris = catatan.isi.trim().split("\n");
...
const grouped: { label: string; isi: string }[] = [];
let currentLabel = "";
let buffer = "";
...
for (const line of baris) {
  ...
  if (labelUtama.includes(label)) {
    if (currentLabel) grouped.push({ label: currentLabel, isi: buffer.trim() });
...
const maxLabelLen = Math.max(...grouped.map((g) => g.label.length));
const labelWidth = doc.getTextWidth("W".repeat(maxLabelLen + 1));
const colonWidth = doc.getTextWidth(": ") + 1;

for (const { label, isi } of grouped) {
  ...
  const wrappedValue = doc.splitTextToSize(
    isi,
    maxWidth - labelWidth - colonWidth
  );
  doc.text(wrappedValue, colonX + colonWidth, currentY);

  currentY += wrappedValue.length * lineHeight + 2;

  if (currentY > yPos + tinggiBaris - 4) break;
}
